...
with open('../../../../resources/query_dict_stopped.txt', "r", encoding='utf-8') as f1:
    queries_dict = ast.literal_eval(f1.read())
    f1.close()

# reads index and makes it a dict
with open('../../../../resources/index_stopped.txt', "r", encoding='utf-8') as f2:
    index = ast.literal_eval(f2.read())
    f2.close()

with open('../../../../resources/docs_length_stopped.txt', "r", encoding='utf-8') as f3:
    dl = ast.literal_eval(f3.read())
    f3.close()


# calculates bm25score and saves it in respective query.txt file
...
def main():
    for query_id in queries_dict.keys():
        ...
        file = open('../../../../output/system_run_output/bm_25_stopped/query_' + query_id + '.txt', 'w')
        rank = 0
