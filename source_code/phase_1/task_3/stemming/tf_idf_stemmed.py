...
def get_index():
    global index_dict
    file = open('../../../../resources/index_stemmed.txt','rU',encoding = 'utf-8')
...
def get_total_tokens():
    ...
    file = open('../../../../resources/docs_length_stemmed.txt','rU',encoding = 'utf-8')
...
def get_queries():
    global query_dict
    file = open('../../../../resources/query_dict_stemmed.txt','rU',encoding = 'utf-8')
        ...
        for term in query_dict[query_id]:
            ...
            if term not in index_dict.keys():
                ...
                for doc_id in index_dict[term]: # doc_id is list of two elements doc id and tf
                    tf = doc_id[1]/total_tokens_dict[doc_id[0]] # '-4 used to remove .txt from filename'
...
def save_top_100():
    ...
    for query_id in query_dict.keys():
        ...
        out_file = open('../../../../output/system_run_output/tf_idf_stemmed/'+'query_'+str(query_id)+'.txt','w')
        rank = 0
        for tuple in sorted_tuples[:100]:
            rank += 1
            out_file.write(str(query_id)+' Q0 '+tuple[0]+' '+str(rank)+' '+str(tuple[1])+' tf_idf_stemmed\n')
    out_file.close()
